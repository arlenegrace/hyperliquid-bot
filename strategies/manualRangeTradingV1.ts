...
export class ManualRangeTradingV1Strategy implements TradingStrategy {
  ...
  evaluate(context: StrategyContext): StrategyResult {
    ...
    if (reclaimEvent.side === "long") {
      ...
      return {
        notes: [
          `${context.symbol}: manual v1 long reclaim confirmed inside range ${formatPerpPriceForConsole(range.low)} - ${formatPerpPriceForConsole(range.high)}.`,
        ],
        signal: {
          strategyId: this.id,
    ...
    return {
      notes: [
        `${context.symbol}: manual v1 short reclaim confirmed inside range ${formatPerpPriceForConsole(range.low)} - ${formatPerpPriceForConsole(range.high)}.`,
      ],
      signal: {
        strategyId: this.id,
        symbol: context.symbol,
